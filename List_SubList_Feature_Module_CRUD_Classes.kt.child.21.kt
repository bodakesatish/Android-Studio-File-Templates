...
class ${NAME}ListViewModel @Inject constructor(
    ...
    private val _navigateToSub${NAME}ListEvent = MutableSharedFlow<String>() // Emits ${NAME.toLowerCase()}Id
    val navigateToSub${NAME}ListEvent: SharedFlow<String> =
        _navigateToSub${NAME}ListEvent.asSharedFlow()    
    ...
    init {
        load${NAME}List(isForcedRefresh = false)
    }

    fun load${NAME}List(isForcedRefresh: Boolean = false) {
    ...
    fun on${NAME}LongClicked(${NAME.toLowerCase()}: ${NAME}) {
        viewModelScope.launch {
            _navigateToSub${NAME}ListEvent.emit(${NAME.toLowerCase()}.uid)
        }
    }
    ...
    fun delete${NAME}(${NAME.toLowerCase()}: ${NAME}) {
        viewModelScope.launch {
            ...
            result.collect { networkResult ->
                ...
                when (networkResult) {
                    is NetworkResult.Success -> {
                        _snackBarMessage.value = "${NAME} '${${NAME.toLowerCase()}.name}' deleted successfully."
                    }

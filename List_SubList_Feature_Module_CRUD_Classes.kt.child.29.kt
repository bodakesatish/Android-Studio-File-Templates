...
class ${NAME}ListFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.swipeRefreshLayout.setOnRefreshListener {
            viewModel.loadCategoryList(isForcedRefresh = true)
        }
    }

    private fun initView() {
        binding.header.tvHeaderTitle.setText("${NAME} List")
    ...
    private fun setupRecyclerView() {
        ${NAME.toLowerCase()}Adapter = ${NAME}Adapter(
            ...
            onItemLongClick = { ${NAME.toLowerCase()} -> viewModel.on${NAME}LongClicked(${NAME.toLowerCase()}) },
    ...
    private fun observeViewModel() {
        viewLifecycleOwner.lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                ...
                // Observe ${NAME.toLowerCase()} list
                launch {
                    viewModel.${NAME.toLowerCase()}List.collectLatest { ${NAME.toLowerCase()}List ->
                        ${NAME.toLowerCase()}Adapter.submitList(${NAME.toLowerCase()}List)
                        binding.textViewEmptyList.isVisible = ${NAME.toLowerCase()}List.isEmpty() && !viewModel.isLoading.value
                        binding.recyclerView.isVisible = ${NAME.toLowerCase()}List.isNotEmpty()
                    }
                }
                ...
                // Observe navigation to Sub${NAME} List
                launch {
                    viewModel.navigateToSub${NAME}ListEvent.collect { ${NAME.toLowerCase()}Id ->
                        val action = ${NAME}ListFragmentDirections.${NAME.toLowerCase()}ListToSub${NAME}List(${NAME.toLowerCase()}Id)
                ...
                launch {
                    viewModel.navigateToAdd${NAME}Event.collect { ${NAME.toLowerCase()}Id ->
                ...
                launch {
                    viewModel.navigateToEdit${NAME}Event.collect { ${NAME.toLowerCase()}Id ->
                        val action = ${NAME}ListFragmentDirections.${NAME.toLowerCase()}ListToAddEdit${NAME}(${NAME.toLowerCase()}Id) // ${NAME.toLowerCase()}Id is null for new
                        findNavController().navigate(action)
                    }
                }
            }
        ...
        binding.swipeRefreshLayout.setOnRefreshListener {
            ...
            viewModel.load${NAME}List(isForcedRefresh = true)
        }
    }
    ...
    private fun showDeleteConfirmationDialog(${NAME.toLowerCase()}: ${NAME}) {
        AlertDialog.Builder(requireContext())
            ...
            .setMessage("Are you sure you want to delete '${NAME.toLowerCase()}.name'?")
